...
function RandomNotes() {
  ...
  const [newBass] = useSound(RandomNotesAudio, {
    playbackRate: 0.25,
    volume: 0.3,
    sprite: theSprites,
  });
  const [newLow] = useSound(RandomNotesAudio, {
    playbackRate: 0.5,
    volume: 0.5,
    sprite: theSprites,
  });
  const [newMid] = useSound(RandomNotesAudio, {
    playbackRate: 1,
    volume: 0.4,
    sprite: theSprites,
  });
  const [newHigh] = useSound(RandomNotesAudio, {
    playbackRate: 2,
    volume: 0.25,
    sprite: theSprites,
  });
  ...
  function startTwoTimer() {
    if (!twoTimer) {
      twoTimer = setInterval(bass, bpmToMS * 2);
    }
  }
  ...
  function startFourTimer() {
    if (!fourTimer) {
      fourTimer = setInterval(low, bpmToMS);
    }
  }
  ...
  function startEightTimer() {
    if (!eightTimer) {
      eightTimer = setInterval(middle, bpmToMS / 2);
    }
  }
  ...
  function start16Timer() {
    if (!sixteenTimer) {
      sixteenTimer = setInterval(high, bpmToMS / 4);
    }
  }
  ...
  return (
    <div>
      ...
      <div>
        <h2 className="border">
          When calling useSound, you can pass options like "playbackRate,"
          "volume," "interrupt," and "sprite" as a second argument:
        </h2>
        <div id="RandomAllFlex">
